...
def live_parse_update_line(line): #this version doesn't check for specific prefixes
    is_withdrawal = False
    valid_flag = True
    path = []
    line_arr = line.split("|")
...
def live_make_diff_udpate_graph(rib_pkl, updateArray, path_file, img_file, output_pkl):
    with open(rib_pkl, 'rb') as handle:
        rib_graph = pickle.load(handle)

    f = updateArray #this is reading from an update file, an array could be placed here to reduce file opening
    f2 = open(path_file, 'r') #this should stay as is
    #the "filtered update data" would already be filtered for
    #the live version, meaning the filtered update data is just new array data thats been formatted
    #with the new implementation, there may not be a need for temp.png, since the
    #png containing old data is overwritten with new data after its used
    #meaning the img file fed in is the one to be overwritten
    #the output pickle will be overwritten
    deleted_edges = []
    new_edges = []
    new_nodes = []
    ...
    for line in f: #for each line in the array, parse the update, this would work for an array
        valid_flag, path, asn, is_withdrawal, curr_prefix = live_parse_update_line(line)
        if valid_flag:
            if is_withdrawal:
                print('FOUND WITHDRAWAL ANNOUNCEMENT')
                target_asn = asn_prefix_map[curr_prefix][0]
...
#This is the same as its static variant, just copied over and changed in case modification is required
def live_make_diff_graph(pkl1, pkl2, img_file): #prefix to asn should be kept constant?
    with open(pkl1, 'rb') as handle:
        rib1_graph = pickle.load(handle)
