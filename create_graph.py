...
def parse_rib_line(line):
    path = []
    line_arr = line.split("|")
    curr_prefix = line_arr[5]#subprefix, just for the purpose of filtering down messages
    if is_subprefix(curr_prefix, des_prefix):
        path = line_arr[6].split()
    return path#list of AS Numbers
...
def make_graph_live(src_file, img_file):
    edges = []
    nodes = []
    ...
    node_colors = ["#A0CBE2" for _ in range(len(nodes))]

    nx.draw_networkx(my_graph, node_color=node_colors, **base_options)
    plt.savefig(img_file)
    
    
make_graph_live("sample_data.json","crap.png")
